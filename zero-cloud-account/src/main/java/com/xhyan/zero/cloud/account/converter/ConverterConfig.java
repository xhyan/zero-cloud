...
public class ConverterConfig {
  ...
  public void initMapping() {
    ...
    List<Class> convertsClasses = AnnotationUtil
        .scanPackages(Converts.class, scanPackages.split(","));
    //解析@Converts注解
    convertsClasses.forEach(clazz -> this.parsingConverts(clazz));
    //获取配置的包路径下@Convert注解的类
    List<Class> convertClasses = AnnotationUtil
        .scanPackages(Convert.class, scanPackages.split(","));
    //解析@Convert注解
  ...
  private ClassMapBuilder parsingConvert(Class clazz) {
    ...
    ClassMapBuilder aToBBuilder = ZeroConvertMapper.MAPPER.mapClass(clazz, target)
        .mapOnNull(converterProperties.isMapOnNull());//映射需要转换的类
    ...
    if (!mapping.equals(AtoBMapping.class) && mapping.isAssignableFrom(AtoBMapping.class)) {
      ...
      Arrays.stream(clazz.getDeclaredFields())
          ...
          .forEach(field -> {
            ...
            if (!transClass.equals(Transformer.class) && transClass
                .isAssignableFrom(Transformer.class)) {
  ...
  private void parsingConverts(Class clazz) {
    ...
    convertList.stream()
        .map(convert -> {
          builderMap.put(convert.group(), ZeroConvertMapper.MAPPER.mapClass(clazz, convert.target())
              .mapOnNull(convert.mapOnNull()));
          return convert;
        })
            ...
            Arrays.stream(clazz.getDeclaredFields())
                ...
                .forEach(field -> {
                  ...
                  Arrays.stream(rules.rules()).forEach(rule -> {
                    ...
                    //注解中了设置targetField，直接取。若未设置，目标属性跟当前属性名称取相同值
                    String targetField = Optional.ofNullable(rule.targetField())
                        .orElse(field.getName());
                    Class<? extends Transformer> transClass = rule.transformer();
                    if (!transClass.equals(Transformer.class) && transClass
                        .isAssignableFrom(Transformer.class)) {
                      builderMap.get(rule.group())
                          .field(field.getName(), targetField, this.getInstance(transClass));
                    } else {
